...
1. **Defines a common interface** (`Datasource`) with five operations: `list`,
   `fetch`, `update`, `close`, and `create`.
...
workflows. Rather than coupling the spec generator and orchestrator to a single
...
| File | Role |
|------|------|
| `src/datasource.ts` | Defines `Datasource`, `IssueDetails`, `IssueFetchOptions`, and `DatasourceName` |
...
| `src/tests/datasource.test.ts` | Test suite covering the md datasource and registry |
...
C4Context
    ...
    System_Boundary(dispatch, "dispatch-tasks") {
        ...
        Container(iface, "Datasource Interface", "src/datasource.ts", "Datasource, IssueDetails, DatasourceName")

        Container(gh_ds, "GitHub Datasource", "src/datasources/github.ts", "Shells out to gh CLI")
        Container(az_ds, "Azure DevOps Datasource", "src/datasources/azdevops.ts", "Shells out to az CLI")
    ...
    System_Ext(github, "GitHub Issues", "Cloud issue tracker")
    System_Ext(azdo, "Azure DevOps", "Cloud work item tracker")
    System_Ext(fs, "Local Filesystem", ".dispatch/specs/ directory")
    System_Ext(git, "Git CLI", "Remote URL inspection")
    ...
    Rel(gh_ds, github, "gh issue view/list/edit/close/create")
    Rel(az_ds, azdo, "az boards work-item show/update/create/query")
...
Every datasource implementation must satisfy this five-method contract defined
in `src/datasource.ts`:
...
All methods accept an optional `IssueFetchOptions` parameter with `cwd`, `org`,
and `project` fields. The `org` and `project` fields are only meaningful for
the Azure DevOps datasource.
...
you must update both the union type in `src/datasource.ts` and the registry map
...
|----------|--------|-----------------|
| Dispatch pipeline (`src/orchestrator/dispatch-pipeline.ts`) | `getDatasource` | `close` (auto-close issues when all tasks succeed) |
| Spec pipeline (`src/orchestrator/spec-pipeline.ts`) | `getDatasource`, `detectDatasource` | `fetch`, `list`, `update` |
| Datasource helpers (`src/orchestrator/datasource-helpers.ts`) | `getDatasource`, `detectDatasource` | Resolution and detection utilities |
...
5. Add tests in `src/tests/datasource.test.ts`.
