...
`mkdir(tmpDir, { recursive: true })`. The temp filename uses a UUID
generated by `crypto.randomUUID()` (e.g., `spec-a1b2c3d4-e5f6-...md`)
to prevent collisions during concurrent batch processing. See
[Node.js crypto integration](./integrations.md#nodejs-crypto-randomuuid).
...
The agent logs the full prompt via the file logger before sending:
`fileLoggerStorage.getStore()?.prompt("spec", prompt)`. This is valuable for
debugging prompt-related generation failures.

### Step 5: Send prompt and read response

The AI provider processes the prompt and (ideally) writes the spec content to
the temp file. The agent logs the response via
`fileLoggerStorage.getStore()?.response("spec", response)`.

**If the provider returns `null`:** The agent immediately returns
`{ success: false }` with an error indicating the provider returned no
response. This can happen when the provider session expires, the model
encounters an error, or the connection is interrupted.

**If the provider throws:** The outer `try/catch` (`src/agents/spec.ts:179-188`)
catches the exception, logs it with a full stack trace via the file logger,
and returns `{ success: false }`. The pipeline's `withRetry()` wrapper will
...
during `dispatch` execution to determine task ordering and concurrency. See
the [markdown syntax reference](../task-parsing/markdown-syntax.md) for the
full checkbox format specification.
...
The spec agent integrates with the
[FileLogger system](./integrations.md#filelogger-and-asynclocalstorage-per-issue-logging)
via `AsyncLocalStorage`. When `--verbose` is enabled, the pipeline scopes a
`FileLogger` to each item's async context, and the agent logs structured
...
- [Integrations & Troubleshooting](./integrations.md) — External dependencies,
  auth, provider failure handling, FileLogger, and randomUUID details
...
- [Logger](../shared-types/logger.md) — How `log.debug()` traces spec
  generation progress and `log.formatErrorChain()` surfaces errors
- [File Logger](../shared-types/file-logger.md) — Per-issue structured
  logging via `AsyncLocalStorage` that the spec agent uses for prompt/response
  recording
- [Configuration](../cli-orchestration/configuration.md) — How `--provider`
  and `--model` configuration flows through to the spec agent
